...
| server run V2 (2026-09-12) | **round 2 — blockfish as label source and second bar**: the cheese-trainer backend layer merged to main (5884c1f: MisaMino/Zetris/Cold Clear/MochBot/blockfish under one `Backend.think` protocol, our pathfinder navigating); `BlockfishAgent` (44719fc) adapts its ctypes shim to the harness `decide(obs)` contract — lock cells matched to our movegen by cell-set identity, legality/navigation stay 100% ours; the shim's per-candidate `rating` (B*-search value, lower = better, ~10 units ≈ 1 piece, terminals exact) exposes blockfish's *within-board* ranking — exactly the signal V1's labels lacked. New harness API `run_episode_from(game, …)` (4594893) plays exact counterfactual continuations from a `Game.clone()` (same RNG stream). Collection (700M band, 974k rows in 10 min, 0 failures): root decisions get **contrast labels** — the played line's realized return, forced top-sibling continuations measured by blockfish play-outs, unvisited candidates get a capped rating-gap prior; non-root rows keep V1's MC returns (the good across-board signal). Two bars on the locked manifest | **Blockfish is the new reference**: L1 1.01 / L3 3.50 / L5 7.24 / **L10 17.13 pieces, 100% win, 1.7 s/ep** — 20% better than lin40x5 (21.47) at 1/10 the time; every future acceptance runs against both bars. **The label fix worked as designed but not far enough**: held-out q-MAE **1.57** (V1: 6.64 — 4× better), within-board pair concordance **0.60** (chance 0.50), teacher's argmin-agreement 16-30% (was ~0). But on the manifest: blend .5 gives L10 **24.81** (100% win) — *worse than V1's 25.89 against the same bar*, and both bars unmet; **pure V still collapses ≥ L3** (L10 0% win, L3 4%, L5 0%). The argmin diagnostic names the failure: 89/215 L10 decisions the net *confidently* prefers a different move >0.5 pieces "cheaper" than the teacher's winning move (mean gap −0.48; p10 −1.13) — it ranks *most* pairs correctly but its worst confusions are catastrophic, and a beam amplifies argmin errors compounding over 17 decisions. The net learned blockfish's *ordering* where labels exist but extrapolates it onto positions the label distribution never visited (labels came from blockfish's own winning lines; V-beam play visits losing positions where no labels exist). Round-3 lever: on-policy relabeling (DAgger-style: play the V-beam, label its *visited* states with blockfish continuations) — the distribution-shift fix that worked for the policy net (run 5), now for V. |
